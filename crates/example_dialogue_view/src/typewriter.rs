...
impl Typewriter {
    pub(crate) fn set_line(&mut self, line: &LocalizedLine) {
        *self = Self {
            ...
            text_direction: self.text_direction, // Preserve text direction
            ..default()
        };
    }

    pub(crate) fn set_text_direction(&mut self, direction: TextDirection) {
    ...
    /// Get the text with proper direction formatting
    fn get_formatted_text(&self) -> String {
        match self.text_direction {
            TextDirection::LeftToRight => self.current_text.clone(),
            TextDirection::RightToLeft => {
                // For RTL, we need to reverse the text
                self.current_text.chars().rev().collect()
            }
            TextDirection::TopToBottom => {
                // For vertical text, add line breaks between characters
                self.current_text.chars().map(|c| c.to_string()).collect::<Vec<_>>().join("\n")
            }
            TextDirection::BottomToTop => {
                // For vertical text bottom-to-top, reverse and add line breaks
                self.current_text.chars().rev().map(|c| c.to_string()).collect::<Vec<_>>().join("\n")
            }
        }
    }

    /// Get the remaining text with proper direction formatting
    fn get_formatted_remaining_text(&self) -> String {
        let remaining = self.graphemes_left.join("");
        match self.text_direction {
            TextDirection::LeftToRight => remaining,
            TextDirection::RightToLeft => remaining.chars().rev().collect(),
            TextDirection::TopToBottom => {
                remaining.chars().map(|c| c.to_string()).collect::<Vec<_>>().join("\n")
            }
            TextDirection::BottomToTop => {
                remaining.chars().rev().map(|c| c.to_string()).collect::<Vec<_>>().join("\n")
            }
        }
    }
}
...
fn write_text(
    ...
    // Update text direction from config if available
    if let Some(config) = config {
        typewriter.set_text_direction(config.text_direction);
    }
    ...
    let current_text = typewriter.get_formatted_text();
    let rest = typewriter.get_formatted_remaining_text();
